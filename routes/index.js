...
router.post('/edit-feature/:id', (req, res) => {
 ...
 router.post('/edit_product', upload.single('productImage'), (req, res) => {
  ...
  let sql = `UPDATE productos SET nombre = ?, codigo = ?, precio = ?, descripcion = ?, categoria_id = ?, caracteristica1 = ?, caracteristica2 = ?, image = ?`;

  db.run(sql, [product.nombre, product.codigo, product.precio, product.descripcion, product.categoria_id, product.caracteristica1, product.caracteristica2, image], 
  (err) => {
    if (err) {
      console.error(err.message);
      res.status(500).send('Server error');
